...
func serve(ctx context.Context, args []string, getenv config.Getenv, stdout, stderr io.Writer) int {
	...
	defer stopReaper()
	handler, err := api.New(api.Options{Controller: control, Verifier: identity.Verifier, Authorizer: identity.Authorizer, MaxBodyBytes: cfg.MaxBodyBytes, MaxUploadBytes: cfg.MaxUploadBytes, Events: emitter})
	if err != nil {
