...
|---|---|
| Commands (`exec`) | yes, without stdin or a terminal |
...
| Interactive terminal (`attach`, `exec` with stdin or a terminal) | no |
...
which the [deploy manifests](../deploy/README.md#the-role) carry. The part
that reaching a port needs is `get` and `create` on `pods/portforward`:
`get` for the WebSocket session current API servers offer, and `create`
for the SPDY upgrade older ones answer instead.
...
written for an earlier release adds the `pods/portforward` rule before it
moves to this one. A worker that runs this runtime checks the same list
when it starts.
