...
environment, `snp_` a snapshot, `msh_` a mesh, `evt_` an event, `op_` a
worker operation, `wrk_` a worker registration, `req_` a request.
...
| A sandbox created on a directly driven environment and one on a worker's environment are indistinguishable through the API except by `status.environment`, `status.driver`, and `status.isolation` | conformance case `case001Indistinguishable` | passing against this server, which runs a `cellad worker` on a second environment beside the one it drives itself for the whole suite run ([[054-environments-desired-state]]) |
