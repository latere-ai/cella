...
| The suite passes against `cellad` on the native tier in under five minutes and against the kind tier with the declared capability set in under fifteen | `TestTheConformanceSuiteHoldsAgainstThisServer`, and `TestContract` in `verify.yml`'s install job and `release.yml`'s conformance job | partial: the native half passes in fifteen seconds against an in-process `cellad serve` with the stubs on every push, 46 passed and 5 skipped ([[059-conformance-closure]]), and against the development stack in the install job. The kind half passed in the install job on 2026-09-22 in four minutes 46 seconds, 33 passed and 18 skipped, with `files` and `pool`; the runs now also pass `mesh`, and `attach` since the Kubernetes driver declares it ([[063-k8s-attach]]), which completes the set that driver declares; the first install job after each is the run that shows it. Both kind runs also declare `display` and `input` and pass the display image, so the computer-use case runs on the cluster, `TestKindStackRunsTheDesktop` ([[064-k8s-display]]). The set this spec's Time section names (the egress modes, `volumes`, `dial`, `resize`) is more than that driver declares, so the kind run cannot assert it yet |
