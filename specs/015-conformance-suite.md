...
| The suite passes against `cellad` on the native tier in under five minutes and against the kind tier with the declared capability set in under fifteen | `TestTheConformanceSuiteHoldsAgainstThisServer`, and `TestContract` in `verify.yml`'s install job and `release.yml`'s conformance job | partial: the native half passes in fifteen seconds against an in-process `cellad serve` with the stubs on every push, 46 passed and 5 skipped ([[059-conformance-closure]]), and against the development stack in the install job. The kind half passed in the install job on 2026-09-22 in four minutes 46 seconds, 33 passed and 18 skipped, with `files` and `pool`; the runs now also pass `mesh`, and `dial` since the Kubernetes driver dials through port forwarding ([[065-k8s-dial]], held by `TestKindRunsDeclareDial`), which completes the set that driver declares without a display image; the first install job after each is the run that shows it. The set this spec's Time section names (the egress modes, `volumes`, `display`, `input`, `attach`, `resize`) is more than that driver declares, so the kind run cannot assert it yet |
