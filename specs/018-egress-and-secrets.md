...
| On k8s, a sandbox reaches an allowed host through its proxy door and cannot reach a disallowed host, another sandbox, or an upstream around the gateway | `TestClusterEgressBoundary`, `TestClusterNoLateralMovement`, conformance case `case018EgressEnforced` | [[070-k8s-egress]]: built, and run on the kind tier of both pipelines; the reverse door on the cluster is open, since it reaches an upstream over TLS on 443 and the tier's upstream serves neither; the Pod network through the gateway is the gateway's own egress rule, which the operator writes |
