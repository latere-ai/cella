...
| [070](.archive/070-k8s-egress.md) | Egress on the k8s driver: a NetworkPolicy per sandbox, the declared modes, the gateway projected into the Pod, mesh peers under confinement, and the kind tier's gateway | large | complete | 004, 012, 013, 015, 018, 022, 031, 036, 039, 065 |
...
| [076](076-rolling-replicas.md) | Rolling replicas: one writer lease gates the controller, every replica answers the API by forwarding to the writer, the lease is handed off on SIGTERM, and readiness means can serve | large | drafted | 005, 008, 010, 014, 017, 018, 021, 043, 072 |
...
| [070-k8s-egress.md](.archive/070-k8s-egress.md) | complete | A NetworkPolicy per sandbox on the k8s driver, ingress from mesh peers alone and, once `CELLA_K8S_GATEWAY_SELECTOR` names the gateway's Pods, egress to DNS, the gateway and mesh peers alone, with `none`, `allowlist` and `open` declared; the gateway projected into the Pod; the kind stack running `cellad egress` and an upstream, and the enforcement case and three cluster tests in both pipelines |
...
| [072-create-answers-at-once.md](.archive/072-create-answers-at-once.md) | complete | A create answers `201` with the sandbox `Pending` once it is recorded, and the scheduler loop finishes it with the driver's create outside the controller's lock and a status write before it; a placed sandbox a restart left is realized by the loop's first pass; `?wait=1` and `timeout` hold the answer, with `client.Wait` and `cella apply --wait`; a boundary no connected gateway can hold is refused as `egress_gateway_unavailable` |
| [073-sandbox-trust-bundle.md](.archive/073-sandbox-trust-bundle.md) | complete | The trust file every driver writes is the public roots and then the gateway's authority, so HTTPS to a host the gateway passes through verifies inside the sandbox; `cellad serve` with a gateway and `cellad worker` read the roots from `SSL_CERT_FILE` or Go's Linux bundle list at start and refuse to start without them; the k8s driver writes the file again at a token rotation and a start |
| [074-client-closed-requests.md](.archive/074-client-closed-requests.md) | complete | A request whose caller closed it while the handler ran, and which then failed with a 5xx, is counted and logged `4xx` `client_closed`; an authorization question its caller abandoned is refused and not counted as `unavailable` |
