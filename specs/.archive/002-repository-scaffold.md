...
| `CELLA_CAPACITY_CPU`, `CELLA_CAPACITY_MEMORY`, `CELLA_CAPACITY_DISK`, `CELLA_CAPACITY_SANDBOXES` | 021 | unset | read by the `worker` role: what one worker declares it can hold, which placement admits against beside the environment's own ceiling; a worker that declares none is admitted against the ceiling alone (051) |
