...
| `TestEveryPipedStepFailsOnTheCommandAndNotTheTee` | a workflow step that reads a command through a pipe without `shell: bash` and `set -o pipefail`, where the default `bash -e` takes `tee`'s exit code and a failing test passes the step |
