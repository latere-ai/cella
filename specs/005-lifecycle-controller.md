...
| For an environment that is `Offline`, nothing is rebuilt, no rule runs, and `Lost` does not count the grace; when it returns, lost sandboxes recover | `TestOfflineEnvironmentHoldsState` | built for the first half as `TestTheReaperHoldsOnAnEnvironmentBelowReady`, over a reaper that now passes per environment ([[054-environments-desired-state]]); the `Lost` grace and the recovery on return are not built |
