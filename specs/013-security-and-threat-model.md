...
| a revoked permission still honoured | a decision is cached per replica for `CELLA_AUTHORIZER_CACHE`, ten seconds by default, which is the accepted window; unavailability is never cached | 006 | `TestDecisionCache` |
...
| an allow without a decision | the authorizer and admission clients fail closed on every non-200, malformed, incomplete, or late answer, with no retry; an `http://` endpoint off loopback is refused at start | 006, 007 | `TestAuthorizerFailsClosed`, `TestAdmissionFailsClosed` |
