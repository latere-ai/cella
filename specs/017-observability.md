...
|---|---|---|
| The registry holds exactly the metrics, labels, and buckets in the table, and every labelled histogram has a series at start | `TestMetricsTable` reading this file through `runtime.Caller` | built for the twenty-one rows whose owning spec has landed, as `TestMetricsTable`, `TestAwaitingRowsAreRegisteredByNobody` and `TestSeriesExistAtStart` ([[053-observability]]); the eight rows of 008's limiter, 020 and 021 are declared in the same table and registered by nothing |
| No label value is a sandbox id, a subject, a name, or a path over a full e2e run | `TestLabelsAreBounded` | built as `TestLabelValuesAreBounded` and the scrape `TestObservabilityEndToEnd` reads after a create, two execs, a stop and a delete ([[053-observability]]) |
| The worker and gateway roles open no metrics listener and their telemetry arrives over OTLP | `TestRolesExportOverOTLP` with an in-memory collector | built for the gateway as `TestEgressRoleServesNoScrapeSurface` and `TestTelemetryExportsOverOTLP` ([[053-observability]]); the worker role lands with [[021-data-plane-workers]] and takes the same seam |
| A request produces one parent span with the route pattern, the request id and trace id as attributes, and the four child spans under stub webhooks; a remote driver call's child span is continued on the worker | `TestRequestSpans`, `TestSpansCrossTheSeam` | the parent span, its name and its attributes are built as `TestRequestSpans` and `TestTheAPIDrawsAServerSpan`; the authorizer and admission children are the instrumented transports' ([[053-observability]]). The seam has nothing to cross until [[021-data-plane-workers]] |
...
| One log line per request, one per stream open and close, none per frame or egress connection | `TestLogVolume` | built as `TestOneLogLinePerRequest` and `TestOneLogLinePerStreamAndNonePerFrame` ([[053-observability]]). A stream is one request and so one line, not two |
