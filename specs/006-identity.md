...
| `limits` override the rate limit, the count ceiling, and the priority cap; `filter` narrows a list | `TestLimitsAndFilter`, `TestCountCeilingCountsEveryDesiredSandbox` | partial: `max_sandboxes` is honoured at create as the count [[007-admission]] defines, and the filter narrows a list; there is still no rate limit ([[008-api]]) and no `Resolve` option carrying `max_priority` ([[003-manifest-contract]]) to override |
