...
| `podman` passes it in the podman tier; `k8s` against kind; `remote` through a worker running `native` | `TestPodmanConformance`, `TestClusterConformance`, `TestWorkerConformance` | `podman` passing against a real engine, the `Attacher` cases included, skipped where no socket answers, [[035-podman-driver]], [[034-terminal-attach]]; `k8s` and `remote` not built |
