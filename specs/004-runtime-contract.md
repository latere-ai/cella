...
| `podman` passes it in the podman tier; `k8s` against kind; `remote` through a worker running `native` | `TestPodmanConformance`, `TestClusterConformance`, `TestWorkerConformance` | `TestClusterConformance` built and skipped where no cluster is configured, [[036-k8s-driver]]; the other two not built |
