...
| Placement admits against the lesser of `spec.capacity` and the live workers' reports; `capabilities` is the intersection | `TestRegistrationMismatch` | the intersection is built as `TestRegistrationMismatch`, [[051-environments-and-workers]]; the capacity half waits on [[020-scheduling-and-sets]]'s admission |
