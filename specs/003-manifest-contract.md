...
| A second YAML document, a wrong version, a wrong kind, and an unsupported content type are refused with their codes, version before kind | `TestDecode` | partial: `TestDecode` over a second JSON document, the version, the kind and a content type that is not JSON |
...
| Every manifest in `testdata/v1/` resolves to its golden output | `TestGoldenCorpus`, `TestCorpusCoversTheSchema` | built ([[056-contract-evidence]]): four accepted manifests with their resolved goldens, fourteen refusals with the code and the paths each earns, under one fixed set of options; every field of the schema is set by some entry and every code a golden records is one this package emits |
