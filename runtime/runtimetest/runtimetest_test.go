...
func TestDeclaredWithoutCaseNamesEveryUncheckedCapability(t *testing.T) {
	if got := declaredWithoutCase(runtime.Capabilities{Files: true, Attach: true, Detach: true, Display: true, Input: true}); len(got) != 0 {
	...
	want := []string{"Egress", "Mesh", "Ingress", "Volumes", "Snapshots", "Dial", "Resize", "Pool"}
