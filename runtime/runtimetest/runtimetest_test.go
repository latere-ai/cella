...
func TestDeclaredWithoutCaseNamesEveryUncheckedCapability(t *testing.T) {
	if got := declaredWithoutCase(runtime.Capabilities{Files: true, Attach: true, Detach: true, Pool: true, Display: true, Input: true, Dial: true}); len(got) != 0 {
	...
	want := []string{"Egress", "Mesh", "Ingress", "Volumes", "Snapshots", "Resize"}
