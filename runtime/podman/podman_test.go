...
func TestDeclarations(t *testing.T) {
	...
	want := driver.Capabilities{Files: true, Detach: true}
	if got := d.Capabilities(); !reflect.DeepEqual(got, want) {
		t.Fatalf("Capabilities = %+v, want %+v", got, want)
	}
	// The forbidden half of the table: this slice advertises none of it.
	c := d.Capabilities()
	if c.Attach || c.Display || c.Input || c.Pool || c.Mesh || c.Dial || c.Volumes || c.Snapshots || c.Ingress || c.Resize || len(c.Egress) > 0 {
