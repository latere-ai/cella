...
// lifecycleOf is the one place the manifest's lifecycle meets the driver's. A
// sandbox that states no lifecycle at all takes the environment's default; one
// that states any bound keeps exactly what it stated, so an explicit never
// stays never rather than picking the default up again.
func (c *Controller) lifecycleOf(obj v1.Sandbox, stated driver.Lifecycle) driver.Lifecycle {
