...
// meshMembershipAt answers MeshMember for the manifest being resolved.
// status is the server's and Resolve zeroes it, so membership is read from
// the three places that carry it at resolve: the declaration a root makes,
// the parent a spawn is held to, and the stored sandbox an update is against.
func meshMembershipAt(obj *v1.Sandbox, o Options) bool {
	switch {
	case obj.Spec.Mesh.Enabled:
		return true
	case o.Parent != nil && MeshMember(o.Parent):
		return true
	case o.Existing != nil && MeshMember(o.Existing):
		return true
	}
	return false
}
